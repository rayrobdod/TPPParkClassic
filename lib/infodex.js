...
(function(){
	...
	window.infodex.displayInfo = function(opts) {
		//if (infodex.currentEntry == opts) return; //this is unlikely to ever work like the other display functions
		
		infodex.clearInfo();
		infodex.fold(false, !!opts.sprite);
		infodex.currentEntry = opts;
	...
	window.infodex.displayPokemon = function(pkmn) {
		if (infodex.currentEntry == pkmn) return; //If same pokemon, don't refresh
	...
	window.infodex.displayTrainer = function(trainer) {
		if (infodex.currentEntry == trainer) return; //If same trainer, don't refresh
		...
		//	}
			...
			var icons = $("#infodex .icon-list").empty();
			if (trainer.icons)
				for (var i = 0; i < trainer.icons.length; i++) {
					var rb = trainer.icons[i];
					if (!(rb instanceof Icon)) continue;
					
					$("<badge>").addClass(rb.cssclass)
						.append(
							$("<img>").attr("src", rb.img).addClass("badgeimg")
						)
						.append(rb.desc)
						.appendTo(icons);
				}
			
			var badges = $("#infodex .badge-list").empty();
			if (trainer.badges && trainer.badges_info)
				var badges_info = trainer.badges_info;
				
				for (var i = 0; i < trainer.badges.length; i++) {
					var rb = trainer.badges[i];
					if (!(rb instanceof Badge)) continue;
					
					$("<badge>")
						.html("<strong>" + rb.name + "</strong><br/>Leader: " + rb.leader + " ("+rb.type+"-Type)")
						.prepend (
							$("<div>").addClass("badgeimg").css({
								"background-image": "url("+badges_info.img+")",
								"background-position": 	"-"+(i*badges_info.frame_width)+"px " +
														"-"+(Math.floor(i/8)*badges_info.frame_height)+"px",
								width: badges_info.frame_width,
								height: badges_info.frame_height,
							})
						)
						.appendTo(badges);
				}
	...
	window.infodex.displayTPPer = function(tpper) {
		if (infodex.currentEntry == tpper) return; //If same tpper, don't refresh
		...
		infodex.currentEntry = tpper;
		
		displaySources("infodex", tpper.sources);
		
		//TODO
	}
	
	window.infodex.displayExtraEntry = function(entry) {
		if (!(entry instanceof InfodexEntry)) return;
		if (infodex.currentEntry == entry) return; //If same entry, don't refresh
		
		infodex.clearInfo();
		infodex.fold(false, !!entry.sprite);
		infodex.currentEntry = entry;
		$("#infodex .page-title").text(entry.title);
		if (entry.gem) $("#infodex .gem").addClass(entry.gem);
		
		infodex.changeHolo(entry.sprite);
		displaySources("infodex", entry.sources);
		
		if (entry.tabs) {
			// For creating tabs
			
		} else {
			// For single pane of info
			var html = entry.html;
			if ($.isFunction(html))
				html = html();
			$("#infodex .sheet").html(html);
		}
	}
...
$(function(){
	var node = $("<div>").addClass("pokeinfo info-sheet");
	
	{
		...
		infonode.append($("<section>").addClass("gender half"));
		infonode.append($("<section>").addClass("game-name"));
		infonode.append($("<section>").addClass("spec-name"));
		
		infonode.append($("<section>").addClass("level half"));
		infonode.append(
			$("<section>").addClass("ballwrap half").append(
				$("<div>").addClass("ball")
			)
		);
...
$(function(){
	var node = $("<div>").addClass("trainerinfo info-sheet");
	
	{
		var infonode = $("<div>").addClass("info");
		node.append(infonode);
		
		infonode.append($("<section>").addClass("trainerid half"));
		...
		infonode.append($("<section>").addClass("released half"));
		infonode.append($("<section>").addClass("caught"));
		
		infonode.append($("<section>").addClass("e4attempts half"));
...
$(function(){
	...
	
	{
		...
		menulist.append(
			$("<a>").text("Mt. Pantheon").attr("href", "#loc=-145,-63").wrap("<li>").parent()
		);
		menulist.append(
	...
	
	{
		...
		window.changelog.forEach(function(message) {
			snode.append( $("<h3>").text(message.date) );
			snode.append( $("<div>").html(message.message) );
		})
...
$(function(){ //jQuery On Ready : Status Dragging
	...
	// $("#infodex .sheet").on("mousewheel", function(e){
	// 	return true; //ALLOW mouse-wheel scrolling
	// });
});
...
$(function(){
	var TIPS = [
		...
		"<p>The arrow keys can move you around the park with speed if you're tried of clicking and dragging! Hold down the arrow keys to accelerate; let go to slide to a stop.</p>",
		//"<p></p>",
		//"<p></p>",
		//"<p></p>",
		//"<p></p>",
